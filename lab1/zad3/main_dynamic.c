...
block *(*dl_allFilesMerge)();
pair *(*dl_fill_pairs_with_data)();
int (* dl_numberOfRowsInBlock)();
void (* dl_deleteBlock)();
void (* dl_deleteRowFromBlock)();
void (* dl_listMainTable)();
...
void run_test(int number_of_blocks, int number_of_rows, FILE *raport){
    ...
    struct tms operation_time[4]; //usr and sys
    ...
    pairs = dl_fill_pairs_with_data(numberOfFiles, fileNames); //poprzez pomiar "mergowania plikow rozumiem czas tworzenia sekwencji par oraz ich mergowania"
    result = dl_allFilesMerge(pairs, number_of_blocks);//number of pairs
    ...
    pairs = dl_fill_pairs_with_data(numberOfFiles, fileNames); 
    result = dl_allFilesMerge(pairs, number_of_blocks);//number of pairs
    for(int i = 0; i < number_of_blocks; i++) {
        dl_deleteBlock(i,result);
    }
...
int main(int no_arguments, char ** arg_list) {
    ...
    //!!!! make test - komenda realizujaca testy i mierzenie czasu
    dl_open= dlopen("./library_shared.so", RTLD_LAZY); //randomowa flaga  prezentacja
    if(dl_open == NULL){
        printf("Can't find lib");
        return -1;
    }
    
    dl_allFilesMerge= dlsym(dl_open, "allFilesMerge");
    dl_fill_pairs_with_data = dlsym(dl_open, "fill_pairs_with_data");
    dl_numberOfRowsInBlock = dlsym(dl_open,"numberOfRowsInBlock");
    dl_deleteBlock = dlsym(dl_open,"deleteBlock");
    dl_deleteRowFromBlock = dlsym(dl_open,"deleteRowFromBlock");
    dl_listMainTable = dlsym(dl_open, "listMainTable");
    ...
    pairs = dl_fill_pairs_with_data(numberOfFiles, fileNames);
    result = dl_allFilesMerge(pairs, numberOfFiles/2);//number of pairs
    //^ to zawsze ma sie wykonac jezeli podamy dobre dane

    printf("%s", "blocks after merging: \n\n");
    dl_listMainTable(result, numberOfFiles/2); //wypisujemy zawartosc po zmergowaniu
    printf("%s","\n");

    while(j<no_arguments){ //sprawdzanie kolejnych argumentow
         if(strcmp(arg_list[j], "remove_block") == 0){
             dl_deleteBlock(strtol(arg_list[j+1],NULL,10),result);
             j++;
         }
         else if (strcmp(arg_list[j], "remove_row") == 0)
         {
             dl_deleteRowFromBlock(strtol(arg_list[j+1],NULL,10),strtol(arg_list[j+2],NULL,10),result);
             j+=2;
         }
         else{
    ...
    dl_listMainTable(result, numberOfFiles/2);
    printf("%s","\n");
